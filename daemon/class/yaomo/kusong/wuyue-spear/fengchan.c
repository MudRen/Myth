...
{
        str="指令：ｐｅｒｆｏｒｍ　ｆｅｎｇｃｈａｎ\n"
        +"条件：基本枪法140级，平天大法100级，五岳神枪100级"
        +"连击技 五岳封禅取义上古人帝对五岳封禅，"
        +"最厉害连续发5招：正气岱宗 平冲中庸 文章衡峰 忠勇西岳 恒岭儒雅，"
        +"非常华丽的绝招，与神力配合使用威力会明显增加，"
        +"最后必杀技 五岳归来不看山 的出现几率有所提高。";
        return str;
}
...
// 同时满足，概率极其微小
                //if(random(ap2+pp)>pp && k>300 && l>180 && random(300)>297 && random(me->query("kar"))>25) {
                  if(random(ap2+pp)>pp && k>300 && l>200 && random(300)>260 && random(me->query("kar"))>18) {
...
void kill_him(object me, object target)
{
        ...
        environment(me) == environment(target))
        {
                ...
                target->receive_damage("sen",target->query("max_sen")+1,me,"pfm");
                target->receive_wound("sen", (int)target->query("max_sen")+1,me,"pfm");
                target->receive_damage("kee",target->query("max_kee")+1,me,"pfm");
                target->receive_wound("kee", (int)target->query("max_kee")+1, me,"pfm");
