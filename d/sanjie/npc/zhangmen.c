...
void init()
{
   ...
   me->create_identity("/d/sanjie/npc/chunyang", "/d/sanjie/tianyidian4");
	...
	if (me->query("current_player") == ob->query("id")) 
	{
        ...
        for(i=0; i<sizeof(inv); i++) {
                ...
                inv[i]->query("equipped") ) 
                        return notify_fail("身上带有高级装备不能打掌门！\n"); 
                else if ( !inv[i]->query_unique() && 
                !inv[i]->query("skill_type") && 
                inv[i]->query("inset_hole") && 
                inv[i]->query("equipped") ) 
  ...
  sscanf(ctime(time()), "%s %s %s %s %s", week, month, day, time, year);  
   if(day=="")  
    sscanf(ctime(time()), "%s %s  %s %s %s", week, month, day, time,year);   
      ...
      if(day!="15" && day!="01" ) 
      { 
      message_vision ("$N皱了皱眉道：今天不是初一和十五比武日，下次在打！\n", me);  
      return 1;} 
