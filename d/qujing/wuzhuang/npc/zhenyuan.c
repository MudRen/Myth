...
{       
   me=this_player();
   if((string)me->query("family/family_name")=="五庄观") {
     message_vision("镇元大仙道：你既然要走，那也不能强留。但是本派的全部特殊武功是五庄观不传之秘，可不能带出观外。\n", me);
...
int do_true(string arg)
{
   if(this_player()->query_temp("asked_leave_wzg") > 1 ) {
     ...
     this_player()->delete_skill("xiaofeng-sword");
     this_player()->delete_skill("sanqing-jian");
     this_player()->delete_skill("zhenyuan-force");
     this_player()->delete_skill("fuqin");
     this_player()->delete_skill("zouxiao");
     this_player()->delete_skill("fumo-zhang");
     this_player()->delete_skill("yange-blade");
     this_player()->delete_skill("wuxing-quan");
