...
void create()
{
        set_name(YEL"元旦巧克力"NOR, ({ "choclate","qiao keli" }) );
        set("long",
HIB"这是雨点为大家准备的元旦巧克力的，颜色鲜艳，让人垂涎欲滴!\n"NOR);
        set("unit", "颗");
        set("weight", 300);
        set("value", 0);
//      set("no_give",1);
...
TEXT,
  ...
  string *actions = ({
   ...
   int bonus;
        ...
        if (this_player()->query("lucky/ny")=="get")
        {
  ...
  message_vision (rdm(color)+"$N"+rdm(hows)+rdm(manners)+rdm(actions)+"。\n"+rdm(color)+rdm(pic)+"\n"+NOR,
          ...
          if (bonus <40000) bonus= 40000;
  this_player()->add("potential",bonus);
  this_player()->add("daoxing",bonus*2);

write(HIR+"你得到了"+chinese_number(bonus)+"点潜能!\n"+
             HIR+"你得到了"+chinese_number(bonus*2)+"点道行!\n",
               environment(this_player()), this_player());
   this_player()->set("lucky/ny","get");
