...
{
       reload("shizhe");
       set_name("帮派使者", ({"boss","cluber","shizhe","waiter"}));
              set("title", "天下楼"); 
       set("shop_id",({"waiter","shizhe","cluber","boss"}));
       set("shop_title","帮派使者");
       set("gender", "男性");
        set("combat_exp", 1000);
       set("age", 26);
...
int accept_object(object who, object ob)
{
    int value;
        ...
        if (ob->query("money_id") && (value=ob->value()) >= 1000000000) 
        {
                tell_object(who, name()+                        "一哈腰，说道：多谢您老，客官请入内。\n");
                who->set_temp("rent_paid",1);
                take_money(value, 1000000000);
                return 1;
        }
        return 0;
}
