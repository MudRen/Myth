...
这是一家从事寄存物品的老字号,江湖上都说这家店铺的信誉高，
所以你大可以放心的把你的东西寄存于此，而且价格还很公道。
你可以在这里寄存(jicun)物品,价格公道,不论物品贵贱，一律
每小时白银一两。若逾期不取(qu)，物品没收后以每样纹银十
两的低价公开出售(pick box)。
...
int do_jicun(string arg)
{
   ...
   if (deposit_time>120 || deposit_time<1 ||!intp(deposit_time)) 
      return notify_fail("本铺寄存的时限只能是一至一百二十小时!\n");
   ...
   if (mapp(treasure_box->query("goods/"+arg)))
      return notify_fail("本铺已经寄存着同样标记的铁盒了,请换个标记再寄存。\n");
...
int do_qu(string arg)
{
   ...
   if (!treasure_box) return notify_fail("对不起,本铺暂停营业!\n");
   if (!arg) return notify_fail("你想取回什么?\n");
   if (!treasure_box->query("goods/number")) return notify_fail("本铺还没有人寄存过东西。\n");
   goods_keys=keys(treasure_box->query("goods"));
   goods_number=sizeof(goods_keys);
   if (goods_number<1) return notify_fail("本铺还没有人寄存过东西。\n");
   for (i=0;i<goods_number;i++)
     {
       if (goods_keys[i]==arg) flag=1;
     }
   if (!flag) return notify_fail("本铺没有人寄存过这个东西!\n");
...
int do_pick(string arg)
{
   ...
   treasure_box=findbox();
   if (!treasure_box)
     return notify_fail("对不起,本铺暂停此种营业!\n");
   if (!treasure_box->query("goods/number")) return notify_fail("本铺暂时还没有可以公开出售的物品。\n");
   goods_keys=keys(treasure_box->query("goods"));
   goods_number=sizeof(goods_keys);
   value=2000;
   if( !(afford=who->can_afford(value)) )  
   ...
   message_vision("本铺暂时还没有可以公开出售的物品。\n",who);   
   return 1;  
}  
