...
void create()
{
  set_name(HIW "龙骑士徽章" NOR, ({"guangming huizhang", "huizhang"}));
  set_weight(50);
  if (clonep())
    set_default_object(__FILE__);
  else {
    set("unit", "枚");
    set("long", HIW"刻有龙骑士头像的魔导徽章\n"NOR);
    set("value", 50000000);
    set("no_sell", 1);
  }
...
int do_zhuan(string arg)
{
  ...
  if (!id(arg))
    return notify_fail("你要转什么职业？\n");
        ...
        message_vision(HIC"$N向光明女神缇雅祈祷，$N成功转职为圣·龙骑士！
\n"NOR, me);  
...
//        me->delete("family");
//        me->delete("class");
        me->set("title", HIW"圣·龙骑士");
        me->delete("betray");
        me->save();
        msg="听说"+ me->query("name")+ "转职为圣·龙骑士了！\n";
