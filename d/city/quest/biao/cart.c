...
int do_open(string arg)
{
        ...
        message("channel:rumor",YEL"『謠言惑众』"+mname
         +"保护的镖车被"+me->query("name")+"劫去了！\n"NOR,users());
...
{        
        ...
        if ( ob_robber = present("robber",environment()))
                if(ob_robber->query("masterid") == me->query("id"))
                        return notify_fail("劫匪伸手一拦道：“想跑？没那么容易！”\n");
...
int check1(object ob, object me)
{
        if (base_name(environment())==(string)me->query_temp("dart_area") 
           ...
           {  
              message_vision("$N累了个半死，终于把镖运到了地头。\n\n",me);
              message_vision(HIG+me->query_temp("dart_name")+
                "拍拍$N的肩膀说道：“"+RANK_D->query_respect(this_player())
                +"辛苦了！”\n\n"NOR,me);
              write(HIG"里间出来几个人将镖车拉了进去。\n"NOR);
...
{
     message_vision("突然骡子大叫一声发疯似拖着镖车跑了。\n",me);
  destruct(me);
  return;
}
...
message_vision(HIR"突然闪出一队人马，为首的头儿大叫道：“来的留下买路钱！。”\n" NOR,
        this_player()); 
