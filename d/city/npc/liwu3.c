...
LONG);
        ...
        "百宝袋"  :(:give_bag:),
            ...
            "东方财神"  : "东方财神他住在深圳，有空可以打电话，tel：0755-6888808",
              "花哥" : "就是koker啊，原来老hlj的空空儿。为了黑龙江的东山再起，他操碎了心，受了大大的冤枉。",
]));
        setup();
...
message_vision("$N对着$n神秘的笑了笑，$n觉得身体发生剧烈的变化，顿时晕倒在地。\n",this_object(),this_player());
        ...
        你的武学增加了十万!\n 
        你的潜能增加了十万点。\n
        您现在有1000点内力和1000点法力，您可以在Enable没有超过上述两值的情况下不用打坐冥思。\n"); 
...
if( who->query("combat_exp") < 600000) {
        tell_object(who,"东北三宝悄悄告诉你：可以向我要礼物哦(ask)(l dongbei sanbao )。\n");  
        tell_object(who,"东北三宝悄悄告诉你：可以向我要门派礼物哦(ask sanbao about family)。\n");  
         }
}
...
int give_bag() 
{
        ...
        if (who->query("daoxing") < 500000 ){  
         message_vision("$N对着$n哼了一声。\n",this_object(),this_player()); 
                command("say 拿百宝袋需要五百年道行，你再努力努力！\n"); 
             return 1; 
             }
             if(present("baibao bag",who)) 
                 {
                 message_vision("$N对着$n哼了一声,你已经有了。\n",this_object(),this_player()); 
                 return 1; 
               }
                newob=new("/obj/sjsh_bag"); 
                ...
                if(newob->restore()) 
                {
                message_vision("$N对着$n神秘的点了点头。\n$N拿出了一个"+newob->name()+"给$n。\n",this_object(),this_player()); 
                newob->move(who); 
                return 1;} 
                else{ 
                command("say 等等，一时半会还没找到！\n"); 
                destruct(newob); 
                return 1;} 
}
