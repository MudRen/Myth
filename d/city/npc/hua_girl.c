...
void create()
{
                  set_name("馨儿", ({ "xin er", "xiner" ,"er" }));
                  set("shen_type", 1);
	set("title", "花店老板娘");
                  set("gender", "女性");
                  set("age", 15);
                  set("long",
"她是这里的老板娘，只见其含笑如春桃，玉唇含香,纤纤细腰,婀娜多姿，
...
int do_full_flower()
{
	...
	tell_room(environment(), "\n周杰伦跟古天乐走了进来。\n");
	 tell_room(environment(), "“嗨！馨儿，给你送鲜花来了。”古天乐与周杰伦对馨儿打招呼。\n");
	 tell_room(environment(), "“馨儿喜声道：“谢谢你们啦，放在后面的花室里吧。”\n");
 	 tell_room(environment(), "周杰伦边哼着：“说不上为什么。我变的很主动，若爱上这馨儿，什么都会值得去做”边去把花摆放好。\n");
	 tell_room(environment(), "周杰伦走了出来对馨儿说道：“花放好了，呵呵。馨儿，你真漂亮。”\n");
	 tell_room(environment(), "古天乐在旁边气的直翻白眼说：“行了吧，你就会简单爱！”\n");
	 tell_room(environment(), "周杰伦对古天乐比划着：哼哼兮兮，我就用双截棍！小心我真敲你。”\n");
	 tell_room(environment(), "周杰伦古天乐恋恋不舍的看着馨儿说：我们走了，美丽的馨儿老板娘。\n");
...
int ask_songhua()
{
        write("馨儿欠身说到：“感谢客官光临小店，您先在这里将花买好，\n"
			  "然后告诉小女要送给谁，小女一定为您送到。\n"
...
int do_song(string arg)
{
	...
	if(me->is_busy())
		...
		if(!send_flower=new("/d/obj/misc/flower2"))
			return notify_fail("/d/obj/misc/flower2初始化错误。\n");
