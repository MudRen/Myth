...
int do_sm()
{
	...
	if( !wizardp(me) ) {
	// Added By waiwai@2001/04/20
	if( !me->query("vip") ) 
		return notify_fail("你不是VIP会员，不能豢养宠物。\n");
  
	//By waiwai@2001/04/20 add 300 yeah
	if( me->query("daoxing") < 100000) 
		return notify_fail("你的道行太低，不能豢养宠物。\n");

	// Added By waiwai@2001/04/20
	if( me->query("combat_exp") < 200000) 
		return notify_fail("你的武学太低，不能豢养宠物。\n");

	if( me->query("limit_pet")>3)
		return notify_fail("你太不爱惜宠物，没有机会再养了。\n");

	// Added By waiwai@2001/04/20
	if( me->query("age") < 16) 
		return notify_fail("你的岁数不到16，不能豢养宠物。\n");
	}
...
/*   
	for(j=0;j<sizeof(bad_word);j++)
		...
		write("\n很抱歉，类似含有 "HIR BLINK+bad_word[j]+NOR" 这样的中文字符被系统禁止注册！\n\n"NOR);
		return 0;
	}
*/
	if( member_array(name, bad_word)!=-1 ) {
...
int  do_dispose(string arg)
{
   ...
   if( !objectp(pet_ob=present(pet_id, ob)) && 
      ...
      // Added by waiwai@2003/05/31
	ridee=ob->ride();

	if(ridee)
      return notify_fail(HIG"你还是先下了坐骑再忙活吧。\n"NOR);
...
void confirm_dispose(string arg, object ob, object pet_ob)
{
   ...
   write("你决定不抛弃你的"+pet_ob->query("name")+"\n");
