...
{
	reload("xiaom_boss");
       set_name("Ğ¡Ã×±¦±¦", ({"xiaom_boss","boss" }));
	set("title", "´ººÍÂ¥ÀÏ°åÄï");
	set("nickname", "ÌÔÆø°ü");
	...
	"here": "ÕâÀïÊÇ´ººÍÂ¥£¬³ÇÖĞ´ó»§ÈË¼Ò½á»é¶¼ÒªÀ´ÕâÀï°ÚÏ²ÑçÁ¨¡£\n",
	"Ñç" : (: ask_party :),
	"Ï²Ñç" : (: ask_party :),
	"Ï¯" : (: ask_party :),
	"¾ÆÏ¯" : (: ask_party :),
	...
	set("no_blade", 1); //²»ÅÂ±ùµ¶
	set("no_poison", 1); //²»ÅÂÓÃ¶¾
	set("no_kill", 1);
	set("limit_fight", 1);
...
//	}
	...
	if( (string)environment(me)->query("short")!="´ººÍÂ¥" ) {
		message_vision("$N¶Ô$nÎªÄÑµÄËµ£ºÏÖÔÚÕâÀïÒ²Ã»×¼±¸£¬ÎÒÊÇÄÑÒÔÏàÖú°¡£¡\n", me, who);
		return 1;
	}
	if( who->query_temp("ready_to_pay") ) {
		message_vision("$N¶Ô$n²»ÄÍ·³µÄËµµÀ£ºÄãÔõÃ´ÎÊ¸öÃ»Íê£¡\n", me, who);
		return 1;
	}
...
int ask_money()
{
	...
	if( who->query("id")!="waiwai"){ 
		command("dunno");
		return 1;
	}
	command("whisper bula Õâ¸öÔÂÉúÒâ²»´í£¬µ½ÏÖÔÚ¹²×¬ÁË"+chinese_number(i)+"Á½½ğ×ÓÁË¡£\n");
	return 1;
}
...
void start_party(object who)
{
	...
	command("chat* £º¸÷Î»ÀÏÒ¯Ì«Ì«ÉÙÒ¯Ğ¡½ã£¬½ñÈÕ" HIR+ log_id(who) +HIC  "ÔÚ´ººÍÂ¥´ó¿ª¾ÆÏ¯£¬»¶Ó­¸÷Î»Ç°À´Åõ³¡£¡"NOR);
...
string ask_duanwu()
{
	...
	who = this_player();
      ...
      if( !who->query("vip") && (int)who->query("combat_exp",1) < 100000 )
		return "ÄãµÄÎäÑ§Ì«µÍ£¬Ã»ÓĞ×Ê¸ñÁìôÕ×Ó£¡£¡£¡\n";

	if( userp(who) ){
		if(who->query("giftZC_2003")) {
		tell_object( who, WHT"Ğ¡Ã×±¦±¦¾ÙÆğÈ­Í·£¬×ĞÏ¸µØ¿´ÁË¿´¡£\n\n" NOR);
		return "ÄãÒª¹ıÁË»¹Òª£¿\n";
	}
	}

	// Added by waiwai@2002/11/27 Ôö¼ÓÃ¿ÄêÖÜÄêµÄÀñÎï£¬Ö»·¢Ò»Ìì
	sscanf(ctime(time()),"%s %s %d %d:%d:%d %d", week, month, day, hour, minute, second, year);

	if( year==2003 && day==4 && month=="Jun" && who->query("giftZC_2003",1)!=1 ) {
	     if( who->query("vip") ) { //»áÔ±ÀñÎïÓë·Ç»áÔ±Çø±ğ
		lp=new("/u/waiwai/gift/zongzi/zongzi");
		lp->move(who);
         	lp->set("owner_id", getuid(who) );
		command("chat* £º¹§Ï²¹§Ï²£¡"HIR+log_id(who)+HIC"µÃµ½ÁËÎ÷ÓÎ¼Ç¶ËÎç½Ú½ğÖÆôÕ×Ó£¡"NOR);
		write(WHT"Ğ¡Ã×±¦±¦µİ¸øÄãÒ»¸ö¶ËÎç½Ú½ğÖÆôÕ×Ó¡£\n"NOR);	
		who->set("giftZC_2003",1);
		log_file("player/2003dw","["+log_time()+"] "+log_id(who)+"µÃµ½ÁËVIPôÕ×Ó¡£\n");
		} else if( !who->query("vip") )  {
		lp=new("/u/waiwai/gift/zongzi/zongzi2");
		lp->move(who);
         	lp->set("owner_id", getuid(who) );
		command("chat* £º¹§Ï²¹§Ï²£¡"HIR+log_id(who)+HIC"µÃµ½ÁËÎ÷ÓÎ¼Ç¶ËÎç½ÚôÕ×Ó£¡"NOR);
		write(WHT"Ğ¡Ã×±¦±¦µİ¸øÄãÒ»¸ö¶ËÎç½ÚôÕ×Ó¡£\n"NOR);	
		who->set("giftZC_2003",1);
		log_file("player/2003dw","["+log_time()+"] "+log_id(who)+"µÃµ½ÁËôÕ×Ó¡£\n");
