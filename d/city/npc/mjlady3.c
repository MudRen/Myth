...
string *MjC = ({"",
                ...
                "Ò»Ìõ","¶şÌõ","ÈıÌõ","ËÄÌõ","ÎéÌõ","ÁùÌõ","ÆßÌõ","°ËÌõ","¾ÅÌõ",
...
//        command("say "+me->name()+"´ò³öÁËÒ»ÕÅ.."+do_check_num(str,0));
        ...
        NowPlayer++;
        NowPlayer%=Play;
        message_vision(YEL"$NËµ£º»¹Ê£ÏÂ["HIC+(end-count-16-Goned)/2+NOR+YEL+"]ÕÅÅÆ¿ÉÃş£¬ÏÖÔÚÂÖµ½ "HIY+
capitalize(NowPlayerId[NowPlayer])+NOR YEL" ÃşÅÆ¡£\n"NOR,this_object());
        if(user = present(NowPlayerId[NowPlayer], environment(this_object())))
        {
        	tell_object(user,HIM"[ ¸ÃÄãÂŞ!! ]\n"NOR);
        }
        if (Auto_Tin){
