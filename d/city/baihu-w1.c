...
/*
	...
	//夏日无雨的夜晚出现纳凉人
	 if( season == "Summer-Wind" || season == "Summer-Sun" ) {
	  set("objects", ([ 
		WAI_NPC"summerman" : 2,
		CITY_NPC"cat" : 1,
	  ]));
	   ...
	   set("objects", ([ //夜晚有小猫出来
		CITY_NPC"cat" : 1,
		CITY_NPC"yanshu" : 1,//鼹鼠与w2做交互出现
	]));
	  } else {//夏秋下雨的白天出现嬉雨顽童
	 if( season == "Summer-Rain" || season == "Autumn-Rain" ) {
	  set("objects", ([ 
		WAI_NPC"kidrain" : 1,
		WAI_NPC"kidgirl" : 1,
		WZG_NPC"zhangguolao": 1,
	  ]));
	} else
*/
	  set("objects", ([ //一般白天出现的Npc
//		WZG_NPC"zhangguolao": 1,
	  ]));
//	}
...
int valid_leave(object me,string dir)
{
	...
	if( !userp(me) && dir=="south" && me->query("race") != "妖魔" ) return 0;

	if(me->query_temp("ridee") && dir=="south" ) 
	return notify_fail(HIG"店小二一把将你推了出来:“对不起, 骑着坐骑不得入内。”\n"NOR);

       if ( dir=="south" ) {
//	if ( present("corpse", me) || present("corpse", environment(me)) )
	if ( present("corpse", me) )
		return notify_fail(HIG"店小二一把将你推了出来：背着个尸体进来干嘛，出去。。。\n"NOR);
		...
		while (sizeinv--) {
       	if (inv[sizeinv]->is_character())  
     		      return notify_fail(HIG"店小二一把将你推了出来：背着别人进来干嘛，出去。。。\n"NOR);
