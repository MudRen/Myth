...
void create()
{
        set_name(BLINK""HIC"金刚石"NOR, ({"bao shi", "baoshi","shi"}));
        set_weight(500);
        if( clonep() )
...
//      return notify_fail("这件装备不能镶嵌宝石。\n");
   ...
   if(!in_obj->query("weapon_prop/damage"))   //防具
  {
       ...
       tell_room(environment(me),HIC"只见"+ob->name()+HIC"砰的一声炸了开来,一团银粉纷纷扬扬的散落在"+in_obj->query("name")+HIC"的周围 \n"NOR);
       in_obj->add("armor_prop/armor",1 + random(10));
       message_vision(CYN""+in_obj->query("name")+CYN"似乎变得更加坚固了！\n"NOR,me);
  ...
  message_vision(CYN"$N把$n"+CYN"镶嵌到"+in_obj->query("name")+CYN"上 \n"NOR,me,ob);
       tell_room(environment(me),HIC"只见"+ob->name()+HIC"砰的一声炸了开来,一团银粉纷纷扬扬的落在"+in_obj->query("name")+HIC"的周围 \n"NOR);
