...
void create()
{
       ...
       set("value", 2000000); 
                ...
                set("no_put",1);
                set("no_sell",1);
                set("long",HIR"一块发着闪闪红光的宝石，传说是女娲补天石的碎块,也不知道什么时候落入凡间的。\n"NOR);
        }
        setup();
}
...
//      return notify_fail("这件装备不能镶嵌宝石。\n");
   ...
   if(!in_obj->query("weapon_prop/damage"))   //防具
  {
       ...
       tell_room(environment(me),HIR"只见"+ob->name()+HIR"化做一道红光,镶入了"+in_obj->query("name")+HIR"里面,不见了! \n"NOR);
...
//武器
       message_vision(CYN"$N把$n"+CYN"镶嵌到"+in_obj->query("name")+CYN"上 \n"NOR,me,ob);
       tell_room(environment(me),HIR"只见"+ob->name()+HIR"化做一道红光,顿时钻进"+in_obj->query("name")+HIR"里面,不见了! \n"NOR);
