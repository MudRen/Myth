...
void create()
{
        set_name(BLINK""HIM"翡翠"NOR, ({"bao shi", "baoshi","shi"}));
        set_weight(500);
        if( clonep() )
               ...
               // set("no_give",1);
       set("value", 2000000); 
                set("long",HIG"一块发着闪闪粉光的翡翠，传说是女娲补天石的碎块,也不知道什么时候落入凡间的。\n"NOR);
        }
        setup();
}
...
//      return notify_fail("这件装备不能镶嵌宝石。\n");
   ...
   if(!in_obj->query("weapon_prop/damage"))   //防具
  {
       ...
       tell_room(environment(me),HIM"只见"+ob->name()+HIM"放出一团朦胧的粉红光,套在"+in_obj->query("name")+HIM"的周围 \n"NOR);
...
//       me->add("bellicosity",1000);  
       ...
       message_vision(CYN"$N把$n"+CYN"镶嵌到"+in_obj->query("name")+CYN"上 \n"NOR,me,ob);
       tell_room(environment(me),HIM"只见"+ob->name()+HIM"放出一团朦胧的粉红光,套在"+in_obj->query("name")+HIM"的上面 \n"NOR);
