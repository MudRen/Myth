...
void create()
{
        set_name(HIW"七星绝尘剑"NOR,({ "juechen sword", "sword", "jian" }) );
        set_weight(2000);
        if( clonep() )
...
int wield()
{
        ...
        if (ob->query("combat_exp") < 100000 
           ...
           message_vision(HIB"\n随着$N一声轻叱，手中寒光闪动，长剑如划破天际的闪电，霎时间，七星闪耀，天地为之变色。\n\n"NOR, ob, me);
           ob->add("force", -200);
           obj = all_inventory(environment(ob));
           if(sizeof(obj) > 1)
               message_vision(HIW"忽然，众人身上所带之剑纷纷自动跃出，插在$N身前的地上，剑尖指天！\n"NOR, ob);
