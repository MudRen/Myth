...
void create()
{
        set_name("铁匠头", ({"tiejiang tou", "tou"}));
        set("age", 52);
        set("title","铁匠协会");
        set("gender", "男性");
        set("int", 35);
        set("long","他是铁匠职业工会负责申请职业的人，要想进入铁匠协会找他没错。\n");
...
void greeting(object who) 
{
        ...
        if( !who->query("occupation") ) {
                tell_object(who,"铁匠头悄悄告诉你：来我们铁匠协会(ask tou about 职业)吧，有很多好处喔。\n");  
        }
}
...
{
        me=this_player();
        if( me->query("occupation")!= 0 && me->query("occupation") != "铁匠"){
                return "你不是已经有职业了吗？\n";
        } 
        ...
        if(me->query("occupation") == "铁匠"){
                write("你想换职业？要换职业请先离开(ask tou about leave)铁匠协会。\n");
                return "三思而后行啊，换职业可是要付出很大代价的喔。\n";
        }
        me->set_temp("need_zhiye", 1);
        write("我这里只能申请【铁匠】这个职业，你确定(yes)要这个职业吗？\n");
        return "欢迎各界人士光临铁匠协会！\n";
}
...
int ask_leave()
{
        ...
        if ( me->query("occupation") == "铁匠" && me->query("work/makeweapon") < 3 ){
                write("打铁技能还没练到家就想离开呀？早了点吧？\n");
                return 1;
        }
        ...
        tell_object(me,"你给了铁匠头一百两黄金。\n"); 
        me->add("balance",-1000000);
        me->delete("work/makeweapon");
