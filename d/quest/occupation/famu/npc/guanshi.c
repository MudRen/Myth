...
void create()
{
        ...
        set("gender", "男性");
        set("int", 35);
        set("long","他是伐木工职业工会负责申请职业的管事先生，要想进入伐木工协会找他没错。\n");
...
void greeting(object who) 
{
        ...
        if( !who->query("occupation") ) {
                tell_object(who,"管事悄悄告诉你：来我们伐木工协会(ask guan about 职业)吧，有很多好处喔。\n");  
        }
}
...
{
        me=this_player();
        if( me->query("occupation")!= 0 && me->query("occupation") != "伐木工"){
                return "你不是已经有职业了吗？\n";
        } 
        ...
        if(me->query("occupation") == "伐木工"){
                write("你想换职业？要换职业请先离开(ask guan about leave)伐木工协会。\n");
                return "三思而后行啊，换职业可是要付出很大代价的喔。\n";
        }
        me->set_temp("need_zhiye", 1);
        write("我这里只能申请【伐木工】这个职业，你确定(yes)要这个职业吗？\n");
        return "欢迎各界人士光临伐木工协会！\n";
}
...
int ask_leave()
{
        ...
        if ( me->query("occupation") == "伐木工" && me->query("work/famu") < 50 ){
