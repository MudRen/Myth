...
void create()
{
        set_name("会长", ({"hui zhang", "zhang"}));
        set("age", 52);
        set("title","商人协会");
        set("gender", "男性");
        set("int", 35);
        set("long","他是商人职业工会负责申请职业的会长，要想进入商人协会找他没错。\n");
...
void greeting(object who) 
{
        ...
        if( !who->query("occupation") ) {
                tell_object(who,"会长悄悄告诉你：来我们商人协会(ask zhang about 职业)吧，有很多好处喔。\n");  
        }
}
...
{
        me=this_player();
        if( me->query("occupation")!= 0 && me->query("occupation") != "商人"){
                return "你不是已经有职业了吗？\n";
        } 
        ...
        if(me->query("occupation") == "商人"){
                write("你想换职业？要换职业请先离开(ask zhang about leave)商人协会。\n");
                return "三思而后行啊，换职业可是要付出很大代价的喔。\n";
        }
        me->set_temp("need_zhiye", 1);
        write("我这里只能申请【商人】这个职业，你确定(yes)要这个职业吗？\n");
        return "欢迎各界人士光临商人协会！\n";
}
...
int ask_leave()
{
        ...
        if (!me->query("occupation") || me->query("occupation") != "商人"){
                write("找错地方了吧？\n");
                return 1;
        }
        if ( me->query("occupation") == "商人" && me->query_skill("trade",1) < 30 ){
                write("经商的本事还没练出来就想走？早了点吧？\n");
                return 1;
        }
