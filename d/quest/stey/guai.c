...
string *idran = ({
       " yao guai"," y_guai"," yaoguai"," yao_guai"," rulai_guai"," yaoguaiyaoguai",
});
...
void setname()
{
        ...
        i=random(sizeof(idran));
        set_name(name, ({ob->query("id") +idran[i]}));
...
int check_result(object me, object ob)
{
    ...
    if (((int)me->query("kee")*100/guai_max_kee) <= 20 )
    {
        ...
        if (random(6) < 1)  command ("say 我还没准备好，你就开打了，不算数。");
           ...
           if(me->query("question")=="done") ob->set_temp("rulai_task",1);
                destruct(me);
                return 1;
            }
...
{       
    ...
    if (me->query("rulaitask/kind") == "杀"  && query("kind") == "杀")
    {
        ...
        else
        {
            ...
            me->delete("rulaitask/get");
            me->set("rulaitask/done",1);
            if(this_object()->query("question")=="done") me->set_temp("rulai_task",1);
            ::die();
        }
    }       
    else 
...
int accept_object (object who, object ob)
{
    ...
    if(guai->query_temp("give_times") < 0 )
    { 
        message_vision(CYN"$N说道：别忙，依次依次来，想毒死我啊。\n"NOR,guai); 
        return 0;
    }
