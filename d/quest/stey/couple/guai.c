...
//object ob = query_temp("last_damage_from"); 
  ...
  object where = environment (me); 
       ...
       ownertwo()->set("job_couple/done"); 
        bons=me->query("job_couple/"+owner()->query("id")+"/rekee")+        me->query("job_couple/"+owner()->query("id")+"/resen");
        bons2=me->query("job_couple/"+ownertwo()->query("id")+"/rekee")+        me->query("job_couple/"+ownertwo()->query("id")+"/resen");
  ...
  call_out("exerthh",1,me); 
      if(objectp(who)  && userp(who)){  
        if(who==owner() && who==ownertwo()) {  
         ...
         if(damage>0) {  
            switch(type) {  
                case "kee": me->add("job_couple/"+who->query("id")+"/rekee",damage); break;  
                case "sen": me->add("job_couple/"+who->query("id")+"/resen",damage); break;  
            }  
         }  
     }  
     }  
