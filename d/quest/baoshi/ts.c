...
void create()
{
        set_name(HIW"唐僧"NOR,({"tang sanzang","sanzang"}));
        set("gender","男性");
        set("age",40);
        set("per", 30);
        set("int", 30);
...
void die ()
{
    ...
    {         
        ...
        message("system",HIG"【"+HIR+"西天取经"+HIG+"】"HIW"如来佛祖"
                HIG"：完蛋了，今儿真是完蛋了！唐三藏在" + query("target") 
                + "被" + ob->query("name") + "吃掉了。\n"NOR, users());
        ob->add("max_force", 5);
        ob->add("max_mana", 5);
        ob->add("maximum_force", 5);
        ob->add("maximum_mana", 5);
        ob->add("age_modify", -8000);
        ob->add("age_modify_time", 1);       
        ob->add("daoxing", 10000);
        ob->add("potential", 2500);
        ob->delete_temp("qujing_kill");        
        ob->save();
        tell_object(ob,"\n你得到了" + chinese_daoxing(10000) + "道行和"
                + "2500点潜能，5点内力和法力，而且又年轻了许多。\n" );
