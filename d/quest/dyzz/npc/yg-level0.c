...
{         
        ...
        damage = guai->query_temp("apply/damage");
        if ( !is_fighting() 
        ...
        if(random(100)>30){
                if( j>=2 ){
                        if (killer->is_team_leader()) {
                                ...
                                tell_object(killer,HIB"你得到了"+COMBAT_D->chinese_daoxing(dx_reward/2)+"道行和"+exp_reward/2+"点武学经验以及"+pot_reward/2+"点潜能的奖励。\n" NOR); 
                        }
                        else {
                                killer->add("combat_exp",exp_reward/2*(j-1));
                                killer->add("potential",pot_reward/2*(j-1));
                                killer->add("daoxing",dx_reward/2*(j-1));
                                tell_object(killer,HIB"你得到了"+COMBAT_D->chinese_daoxing(dx_reward/2*(j-1))+"道行和"+exp_reward/2*(j-1)+"点武学经验以及"+pot_reward/2*(j-1)+"点潜能的奖励。\n" NOR); 
                        }
                }
                        ...
                        tell_object(killer,HIB"你得到了"+COMBAT_D->chinese_daoxing(dx_reward)+"道行和"+exp_reward+"点武学经验以及"+pot_reward+"点潜能的奖励。\n" NOR); 
                }
        } 
        if(random(100)>95){
                if( j>=2 ){
                        if (killer->is_team_leader()) {
                                ...
                                tell_object(killer,HIB"你得到了"+MONEY_D->money_str(gold_reward/2*(j-1))+"的奖励。\n" NOR); 
                        }
                }
                else if(!pointerp(killer->query_team())){
