...
//      set_name(w_name, ({w_id}));
        ...
        switch(w_or)
                ...
                case "千年神木" :
                        ...
                        set("wield_msg", "$N缓缓地从背后出拿出一根" + w_name + "。\n");
...
int do_duanlian(object weapon)
{
       ...
       if( (int)me->query("weapon/lv") > 21)
        ...
        if( (int)me->query("maximum_force") < 2200 )
                return notify_fail("你的内力上限不够，无法锻炼枪劲！\n");

        if( (int)me->query("max_force") < 2200 )
                return notify_fail("你的内力不够，无法锻炼枪劲！\n");

        if( (int)me->query("kee") < 300 )
                return notify_fail("你的气不够，无法锻炼枪劲！\n");

        if( (int)me->query("eff_kee") < 300 )
                return notify_fail("你现在的体力太弱，无法锻炼枪劲！\n");

        if( (int)me->query("eff_sen") < 300 )
                return notify_fail("你现在的精力无法集中，不能锻炼枪劲！\n");

        if( (int)me->query("sen") < 300 )
                return notify_fail("你现在的精神太弱，无法锻炼枪劲！\n");

        if( ((int)me->query("potential") - (int)me->query("learned_points"))< 100 )
                return notify_fail("你的潜能不够，无法锻炼枪劲！\n");

        message_vision(HIR "$N手握枪身，一股内力丝丝的传了进去。\n" NOR, me);
        ...
        if( w_zhi >= ((w_level + 1) * (w_level + 1)))
        {
                ...
                message_vision(HIY "枪身忽的一亮，一道金光隐入$N的枪身，不见了！\n" NOR,me);
                message_vision(HIG "$N的枪的等级提高了！\n" NOR, me);
                weapon=this_object();
...
{       
        ...
        message_vision(HIR "$N大喝一声，一掌擎枪，一掌猛力击下。结果轰隆一声巨响"
                        +w_name+"断为两截!\n" NOR,me);
        me->delete("weapon");
        me->delete("bld/done");
