...
//      set_name(w_name, ({w_id}));
        ...
        switch(w_or)
                ...
                case "千年神木" :
                        ...
                        set("wield_msg", "$N往怀中一摸，掏出了一条"+MAG""+w_name+""+NOR"，端在手中。\n");
...
int do_duanlian(object weapon)
{
       ...
       if( (int)me->query("weapon/lv") > 21)
        ...
        if( (int)me->query("maximum_force") < 2200 )
                return notify_fail("你的内力上限不够，无法锻炼棍体！\n");

        if( (int)me->query("max_force") < 2200 )
                return notify_fail("你的内力不够，无法锻炼棍体！\n");

        if( (int)me->query("kee") < 300 )
                return notify_fail("你的气不够，无法锻炼棍体！\n");

        if( (int)me->query("eff_kee") < 300 )
                return notify_fail("你现在的体力太弱，无法锻炼棍体！\n");

        if( (int)me->query("eff_sen") < 300 )
                return notify_fail("你现在的精力无法集中，不能锻炼棍体！\n");

        if( (int)me->query("sen") < 300 )
                return notify_fail("你现在的精神太弱，无法锻炼棍体！\n");

        if( ((int)me->query("potential") - (int)me->query("learned_points"))< 100 )
                return notify_fail("你的潜能不够，无法锻炼棍体！\n");

        message_vision(HIR "$N手握棍体，一股内力丝丝的传了进去。\n" NOR, me);
