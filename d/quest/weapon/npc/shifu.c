...
{       
        ...
        message("vision","侍斧低头检视了一下手中的"+(string)ob->query("name")+
                        "点了点头说：\n", who);
...
int do_zhu(string arg)
{
        ...
        int i;
        me = this_player();
        if(!(me->query_temp("m_axe")) ) 
                return notify_fail("什么?\n");
        me->delete_temp("m_axe");
        me->delete_temp("get_orc");
        me->delete_temp("or");

        if((me->query("weapon/make")) ){
                say("侍斧一脸茫然：您已经有一把自造的武器了，还来干什么？难到不可手么？\n");
                return 2;
        }
        if( !arg )
                return notify_fail("侍斧认真的说：想好斧的名称及代号后在对我说。\n");

        sscanf(arg ,"%s %s" ,w_name ,id);
        if(!w_name||!id)
                return notify_fail("什么?(使用zhu 斧的名字 英文代号\n)");
        i = strlen(id);
        while(i--)
                if( id[i]<'a' || id[i]>'z' )
                        return notify_fail("对不起，斧的英文代号只能用英文字母。\n");
        ...
        message_vision(me->name()+"沉吟了一会，对侍斧巧声说了几句话。侍斧点了点头。说：好吧！\n",me );
        message_vision("\n侍斧回过身，转向身后的一个巨大的火炉，鼓动真气燃起了熊熊的大火。说：开始！\n",me );
        ...
        message_vision(HIR "只听咯的一声轻响，一柄巨斧从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
        say("侍斧见状大叫：神斧初成，人血以祭！！闪开！\n");
        message_vision("$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n",me );   
        message_vision("巨斧透胸穿出，侍斧惨号一声，鲜血溅得你满脸都是！\n",me );
        message_vision(RED "侍斧脚下一个不稳，倒在了地上。侍斧已经奄奄一息了。\n" NOR,me);
        this_object()->set("eff_kee",10);
        message_vision("巨斧又飞了起来，飞到半空，当的一声落回到地上。\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
        ...
        club=new("/d/quest/m_weapon/weapon/m_axe",1);    //生成新斧
        club->move(this_player());
        message_vision("侍斧摸起地上，带着斑斑血迹还有些烫手的巨斧，说：\n斧。。已。。成。。，侍斧的任务。。。也就完成了。。。。\n",me );
        say("侍斧艰难的说：斧。。您。。收好，我该走了。。。\n");
        message_vision("侍斧说完。倏的便不见了。\n",me);
