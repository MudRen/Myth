...
void create()
{
        ...
        set("inquiry", ([
                "铸锏"  : "师傅命我帮助别人铸锏，但到现在也不见一个人影。谁要铸锏呢？\n",
                "原料"  : "铸锏可用寒丝羽竹，海底金母，千年神木。但效果各不一样。\n",
        ]) );
...
{       
        ...
        message("vision","侍锏低头检视了一下手中的"+(string)ob->query("name")+
                        "点了点头说：\n", who);
        message("vision","。。。好我们现在就开始铸锏\n", who);

        write("请这位"+RANK_D->query_respect(who)+"给锏起名字吧！(zhu 锏的名字 英文代号)\n");
                remove_call_out("destrory");
                call_out("destrory",1,ob);
        return 1;
...
int do_zhu(string arg)
{
        ...
        int i;
        me = this_player();
        if(!(me->query_temp("m_mace")) ) 
                return notify_fail("什么?\n");
        me->delete_temp("m_mace");
        me->delete_temp("get_orc");
        me->delete_temp("or");

        if((me->query("weapon/make")) ){
                say("侍锏一脸茫然：您已经有一把自造的武器了，还来干什么？难到不可手么？\n");
                return 2;
        }
        if( !arg )
                return notify_fail("侍锏认真的说：想好锏的名称及代号后在对我说。\n");

        sscanf(arg ,"%s %s" ,w_name ,id);
        if(!w_name||!id)
                return notify_fail("什么?(使用zhu 锏的名字 英文代号\n)");
        i = strlen(id);
        while(i--)
                if( id[i]<'a' || id[i]>'z' )
                        return notify_fail("对不起，锏的英文代号只能用英文字母。\n");
        ...
        message_vision(me->name()+"沉吟了一会，对侍锏巧声说了几句话。侍锏点了点头。说：好吧！\n",me );
        message_vision("\n侍锏回过身，转向身后的一个巨大的火炉，鼓动真气燃起了熊熊的大火。说：开始！\n",me );
        ...
        message_vision(HIR "只听咯的一声轻响，一柄巨锏从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n" NOR,me );
        say("侍锏见状大叫：神锏初成，人血以祭！！闪开！\n");
        message_vision("$N只觉得眼前一花，一条白影迅捷无比的挡在了$N的身前。\n",me );   
        message_vision("巨锏透胸穿出，侍锏惨号一声，鲜血溅得你满脸都是！\n",me );
        message_vision(RED "侍锏脚下一个不稳，倒在了地上。侍锏已经奄奄一息了。\n" NOR,me);
        this_object()->set("eff_kee",10);
        message_vision("巨锏又飞了起来，飞到半空，当的一声落回到地上。\n炉中的火灭了。一室的劲气化于无形，一切又归于沉寂。\n",me );
        ...
        club=new("/d/quest/m_weapon/weapon/m_mace",1);    //生成新锏
        club->move(this_player());
        message_vision("侍锏摸起地上，带着斑斑血迹还有些烫手的巨锏，说：\n锏。。已。。成。。，侍锏的任务。。。也就完成了。。。。\n",me );
        say("侍锏艰难的说：锏。。您。。收好，我该走了。。。\n");
        message_vision("侍锏说完。倏的便不见了。\n",me);
