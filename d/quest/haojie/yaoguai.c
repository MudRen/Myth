...
//		    if(ratio<0) ratio=0;
		    ...
		    str="kill xinmo got "+exp_r+"/"+exp_reward+" daoxing "+
                    ...
                    MONITOR_D->report_quest_object_msg(who, str);

		    who->add("daoxing",exp_r);
		    who->add("potential",pot_r);
              who->add_temp("hj_killer",1);
		    who->delete("xinmo");
        who->add("balance",10000);
        tell_object(who,"你的存款增加了一两黄金。\n");
        CHANNEL_D->do_channel( this_object(), "job",
        sprintf("攻打%s的"NOR+YEL"<%s>"CYN"被%s杀死了。"NOR,query("family/family_name"),query("name"), ob->name() ));
