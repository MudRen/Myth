...
string zname(object);
#include <ansi.h>
#define DIRNAME "/d/gumu/npc/"

#define MYFILE "/d/gumu/npc/zhangmen.c"
...
string zm_apply()
{
        ...
        if (me->query("current_player") == ob->query("id"))                 command("say 好！我们便切磋一下，尚请未来的"+zname(ob)+"手下留情。\n");
        }
        else
        {
        ...
        for(i=0; i<sizeof(inv); i++) 
        {
                ...
                if (who && who->query("id")!=me->query("current_master"))
                        who = 0;        ob->delete_temp("moon_zm_applied");
        me->set("new_player",1);
...
void create_identity (mixed master, mixed where)
{
        ...
        if ( (string)ob->query("gender")=="男性" 
                ...
                return "师傅不喜欢男性，男性很难做掌门的，我劝你还是死心吧！\n"
        
        ob->set_temp("moon_zm_applied", 1)
        return "能做轩辕古墓的主人，还要看看你有没有这个本事了。\n想做古墓主人的位子不难，胜了我你就是新的主人！\n"
...
int check_result(object me, object ob
        ...
        if ( !living(me)
       message_vision ("$N躬身对$n说道：恭请见过妲己。\n",me,ob)
...
void find_master (object me, object ob
        ...
        els
                ...
                message_vision ("$N见到妲己低声说了几句话，又抬起头朝$n使了个眼色。\n\n",me,ob)
...
*
        return HIW"轩辕古墓主人"NOR ; //for moon only, others use above
int rank_me(
