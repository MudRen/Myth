...
#define MYFILE "/d/gumu/npc/zhangmen1.c"
...
void init()
{
	...
	object who = this_player();
        ...
        me->create_identity("/d/gumu/npc/daji", "/d/gumu/qianting");
...
string zm_apply()
{
	...
	if (me->query("current_player") == ob->query("id")) 
	{
        ...
        ob->set_temp("gumu_zm_applied", 1);
        return "如有意夺取掌门一位，还请与我一战。\n";
}
...
int accept_fight(object ob)
{
	...
	if (me->is_fighting())
	{
        ...
        if (ob->query_temp("gumu_zm_applied"))
	{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("pansi_zm_applied"))
		{
			...
			return 1;
                ...
                if( ob->query_temp("gumu_zm_applied") != 1 )
			return 1;	
...
void master_announce (object me, object who, object ob)
{
	CHANNEL_D->do_channel(who,"chat","本派掌门弟子"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
        ob->delete_temp("gumu_zm_applied");
	me->set("new_player",1);
