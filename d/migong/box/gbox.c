...
{ 
       ...
       ||environment(me)->query("short")==CYN"石洞"NOR)
    {
        ...
        && !environment(this_object())->query("alrd"))
        {
                 ...
                 message("channel:chat", HIM"【龙窟谣言】听说"+me->name() + "在龙窟迷宫中找到了一颗"+corpse->query("name")+HIM"!"NOR"。\n", users());
                 corpse->move(me);
       ...
       ||environment(me)->query("short")==CYN"石洞"NOR)
    {
        ...
        && !environment(this_object())->query("alrd"))
        {
             ...
             message("channel:chat", HIM"【龙窟谣言】听说"+me->name() + "在龙窟迷宫中找到了一枚"+corpse->query("name")+HIM"!"NOR"。\n", users());
             corpse->move(me);
             environment(this_object())->set("alrd",1);
      ...
      else if(random(200)>198)
      {
        ...
        me->add("kee",-10);
        tell_object(me,"宝箱里发出数个"+RED"暗器,你中了蛇毒!"NOR+"。\n"NOR);
      }  
      else if(random(200)>198)
      {
        me->apply_condition("rose_poison",10);
        me->add("kee",-10);
        tell_object(me,"宝箱里发出数个"+RED"暗器,你中了火玫瑰毒!"NOR+"。\n"NOR);
      }  
      else if(random(200)>198)
      {
        me->apply_condition("hell_zhua",10);
        me->add("kee",-10);
        tell_object(me,"宝箱里发出数个"+RED"暗器,你中了尸毒!"NOR+"。\n"NOR);
      }  
      else if(random(200)>198)
      {
        me->apply_condition("bp_poison",10);
        me->add("kee",-10);
        tell_object(me,"宝箱里发出数个"+RED"暗器,你中了冰毒!"NOR+"。\n"NOR);
      }  
      else if(random(200)>198)
      {
        me->apply_condition("double_ice_poison",10);
        me->add("kee",-10);
        tell_object(me,"宝箱里发出数个"+RED"暗器,你中了寒毒!"NOR+"。\n"NOR);
      }  
      else if(random(200)>198)
      {
        me->apply_condition("xiangsi",10);
        me->add("kee",-10);
        tell_object(me,"宝箱里发出数个"+RED"暗器,你中了情毒!"NOR+"。\n"NOR);
      }  
      ...
      {
        tell_object(me,"宝箱里发出一道"+HIR"血光,你觉的事情不好了!!"NOR+"。\n"NOR);
        me->die();
      }  
      else if(random(200)>198 && environment(me)->query("short")==BLU"山洞"NOR)
      {
        tell_object(me,"宝箱里发出一阵详和的"+HIY"金光,你的经验和道行增加了!!"NOR+"。\n"NOR);
      ...
      {
        tell_object(me,"宝箱里发出一阵详和的"+YEL"金光,你的内力和法力增加了!!"NOR+"。\n"NOR);
        me->add("max_force",10);
        me->add("max_mana",10);
      }  
      else if(random(200)>198 && environment(me)->query("short")==RED"熔岩洞"NOR)
      {
        tell_object(me,"宝箱里发出一阵详和的"+YEL"金光,你的经验，道行，潜能，内力和法力都增加了!!"NOR+"。\n"NOR);
