...
三界中最凶险的地方[眠龙洞]。传说洞里藏着无数宝物，数万年来无数
仙魔鬼怪葬送于此，但活着出来的人却从没有走到过洞的尽头。
那里似乎有一个闪光点，你好像通过这里移到(yidong)什么地方去。 
LONG );
        set("exits", ([
...
int valid_leave(object me, string dir)
{
  ...
  {    
        if ( me->query("combat_exp") < 5000000)
                return notify_fail("你这么微末的道行进去不是送死吗？\n");
                ...
                while (sizeinv--)
                {
                        if (inv[sizeinv]->is_character())  
                                return notify_fail("洞里的怪物可饿着呢，你想给他们当点心？\n");
...
object me=this_player();
        ...
        return notify_fail("你没有使用过天地宝鉴在迷宫里记录你所到过的层次！\n");
                inv=all_inventory(me);
                sizeinv=sizeof(inv);
...
{
        message_vision(HIG "一团奇异的光辉笼罩了$N!\n" NOR, me);
i=random(100)+1;
...
me->move(mapm);
        ...
        if(arg=="lev15") {
        message_vision(HIG "这层不能直接到达，请到lev14!\n" NOR, me);
}
