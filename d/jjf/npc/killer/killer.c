...
{       
      ...
      object ts = present ("tang xuanzhang", where); 
      object ts2 = find_living("tang xuanzhang"); 
   
   if (!ts2) 
       {    
        
        message("vision",HIB + name() + "“ª¡Ô—Ãµƒ≈‹¡À°£\n" NOR,environment()); 
        ...
        if(ts!=owner())destruct (this_object()); 
        ts->set("can_move",1);
