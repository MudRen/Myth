...
void create()
{
        set_name(HIM"碧海令"NOR,({"bihai necklace","necklace","club_mark","sword","jian"}));
        set_weight(0);
        ...
        set("long",@long
              * 碧海晴天 **
一块天青色的令牌！左边刻一青龙，右边刻一白虎！正中写着
** 碧海晴天 **四个大字！让人看了不寒而栗！
你可以用(help bihai)来查看有什么碧海阁指令。若想了解碧海阁
的历史可以用(about bihai)来取得进一步的资料。

LONG);
        }
        set("club_id",CLUB_ID);
        set("club",CLUB_NAME);
        set("sub_club",({"星月堂","御剑堂","凝香堂","神风堂","狂影堂","炎狼堂","縹緲堂","地獄堂",}));
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】碧海阁荣誉帮众  ";
 ...
 switch(rank)
 {
        case 10: str+="碧海阁幫主     "; break;
        case 9: str+="碧海阁副幫主    "; break;
        case 8: str+="碧海阁總護法    "; break;
        case 7: str+="碧海阁左護法    "; break;
        case 6: str+="碧海阁右護法    "; break;
        case 5: str+=info["info1"]+"分堂主總管  "; break;
        case 4: str+=info["info1"]+"副堂主劍仙  "; break;
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        ...
                        if( (int) member_array(sub, query("sub_club"))<0 )
                                return notify_fail("目前可用的分堂为 【星月堂】、【御剑堂】、【凝香堂】、【神风堂】、【狂影堂】、【炎狼堂】、【縹緲堂】、【地獄堂】\n");
                        ob_info["level"]=5;
                        ob_info["info1"]=sub;
                        CLUB_D->set_member(CLUB_ID,getuid(ob),ob_info);
                        if(!mark=present(CLUB_MARK,ob))
...
int do_givepower(string arg)
{
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        ...
                        if(level > 9 || level < 1 )
                                return notify_fail("目前等级分为<1:刺客> <2:剑客> <3:剑神> <4:劍仙> <5:總管> <6:堂主> <7:左護法> <8:右護法> <9:總護法> <10:幫主> \n");
                        if( level > ob_info["level"] )
                        {
                                type="提升";
                                res ="并和$n握手勉励一番";
...
int do_handover(string arg)
{
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        ...
                        message( "channel:bihai",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")将"+CLUB_NAME+"指挥权移交给"+ob->name(1)+"("+capitalize(ob->query("id"))+")。\n"NOR, users() );
                        ob->save();
                        me->save();
                        return 1;
                }
...
int do_leaveclub(string arg)
{
        ...
        message( "channel:bihai",CYN"【"+CLUB_NAME+"】"+me->name(1)+"("+capitalize(me->query("id"))+")主动退出了"+CLUB_NAME+"。\n"NOR, users() );
 ...
 if(arg=="bihai")
 {
  write(@HELP
       ...
       └───────────────┴─────────┴──────┘
         ...
         ◎ 等级分为: <1:刺客> <2:剑客> <3:剑神> <4:劍仙> <5:總管>
                     <6:堂主> <7:左護法> <8:右護法> <9:總護法> <10:幫主>。
