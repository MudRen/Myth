...
#include <ansi.h>
#include <weapon.h>
#define CLUB_NAME "忘忧谷依轩楼"
#define CLUB_ID "wangyou"
#define CLUB_CHANNEL "wangyou"
#define CLUB_MARK "wangyou necklace"
inherit SWORD;
...
void create()
{
         set_name(HIG"忘忧谷依轩楼"NOR,({"wangyou necklace","necklace","club_mark","sword","jian"}));
        set_weight(0);
...
这是一个玉佩令牌，隐隐幻出丝丝荧光，此乃忘忧谷依轩楼之信物。
你可以用(help wangyou)来查看有什么忘忧谷依轩楼指令。若想了解忘忧谷依轩楼
的历史可以用(about wangyou)来取得进一步的资料。
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】忘忧谷依轩楼荣誉帮众  ";
 ...
 switch(rank)
 {
        case 10: str+="无忧楼主      "; break;
        case 9: str+="忘忧谷依轩楼管家  "; break;
        case 8: str+="忘忧谷依轩楼楼主仕女    "; break;
        case 7: str+="忘忧谷依轩楼楼主护卫  "; break;
        case 6: str+="忘忧谷依轩楼资深妖月仕女  "; break;
        case 5: str+=info["info1"]+"圣阁护卫    "; break;
        case 4: str+=info["info1"]+"莲星仕女    "; break;
        case 3: str+="忘忧谷依轩楼御庭护卫      "; break;
        case 2: str+="忘忧谷依轩楼护院      "; break;
...
*/
                if(sizeof(ob_info))
                {
                        if(me_info["level"] != 5 ) return notify_fail("只有妖月仕女能将帮众收入分堂。\n");
                        if(!stringp(sub=me_info["info1"])) return notify_fail("只有妖月仕女能将帮众收入分堂。\n");
                        if(ob_info["level"] != 2 || ob_info["info1"]!="无")
                                return notify_fail("只有一般帮众可以收入你的部门。\n");
                        ob_info["info1"]=sub;
...
int do_dismiss(string arg)
{
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        if(!sub=me_info["info1"] || me_info["level"]!=5 ) return notify_fail("只有管家能免除堂员职务\n");
                        if(ob_info["level"] != 1 ) return notify_fail("你不够资格解除对方的职务。\n");
                        if(ob_info["info1"] != sub) return notify_fail("对方并不属于你的部门。\n");
                        ob_info["info1"]="无";
                        CLUB_D->set_member(CLUB_ID,getuid(ob),ob_info);
...
*/
                 ...
                 else
                {
                 ...
                 mark->move(ob);
                 ob->add("channels", ({"wangyou"}));
                 ob->add("club", "忘忧谷依轩楼");
                 ob->add("title", HIC"忘忧谷依轩楼" NOR);
                 ob->save();
                }
        return 1;
}
...
*/
                if(sizeof(ob_info))
                {
                        ...
                        {
                                message_vision("$N将$n提升为"+sub+"莲星仕女, 并和$n握手勉励一番。\n",me,ob);
                                mark->move(ob);
                        }
                        ob->save();
                        return 1;
...
int do_givepower(string arg)
{
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        ...
                        if(level > 9 || level < 1 )
                                return notify_fail("目前等级分为 <1:丫鬟> <2:护院> <3:御庭护卫> <4:莲星仕女> <5:圣阁护卫> <6:妖月仕女> <7:楼主护卫> <8:楼主仕女> <9:管家> <10:无忧楼主> \n");
                        if( level > ob_info["level"] )
                        {
                                type="提升";
                                res ="并和$n握手勉励一番";
...
int do_help(string arg)
{
 if(arg=="wangyou")
 {
  write(@HELP
       ...
       │  members                     │列出线上帮会成员  │护院        │
       │  leaveclub                   │主动退出帮会      │护院        │
       │  include <人名>              │将某人收入分堂    │圣阁护卫专用│
       │  dismiss <人名>              │取消某人分堂职务  │楼主护卫专用│
       │  accept <人名>               │让某人加入帮会    │管家专用    │
       │  kickout <人名>              │将某人逐出帮会    │管家以上    │
       │  subleader <人名> to <分堂名>│升级某人为分堂主  │楼主侍女以上│
       │  retire <人名>               │撤除某人分堂主职位│妖月侍女以上│
       │  givepower <人名> to <等级>  │改变某人的职位    │管家以上    │
       │  handover <人名>             │掌门职务交接      │楼主侍女    │
       │  listmember                  │帮会名册          │楼主护卫以上│
       └───────────────┴─────────┴──────┘
         ◎ 使用 accept 指令必须对方先用 joinclub 指令申请才能生效。
         ◎ 清风细雨楼目前有四个分堂: 
         【无忧堂】、【无恨堂】、【无情堂】、【无我堂】
        
       ◎ 等级分为: <1:丫鬟> <2:护院> <3:御庭护卫> <4:莲星仕女> <5:圣阁护卫> <6:妖月仕女>
                　　　<7:楼主护卫> <8:楼主仕女> <9:管家> <10:无忧楼主>。
