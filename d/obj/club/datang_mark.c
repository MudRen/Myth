...
#include <ansi.h>
#include <weapon.h>
#define CLUB_NAME "梦回唐朝"
#define CLUB_ID "datang"
#define CLUB_CHANNEL "datang"
#define CLUB_MARK "datang necklace"
inherit SWORD;
...
void create()
{
         set_name(HIY"梦回唐朝阁的信物"NOR,({"datang necklace","necklace","club_mark","sword","jian"}));
        set_weight(0);
...
这是一个五彩玉佩令牌，隐隐幻出字型，此乃梦回唐朝阁之信物。
你用( Help Datang )来查看梦回唐朝阁指令。若想了解梦回唐朝阁
的历史可以用(About Datang)来取得进一步的资料。
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】梦回唐朝阁荣誉帮众  ";
 ...
 switch(rank)
 {
        case 10: str+="梦回唐朝阁阁主      "; break;
        case 9: str+="梦回唐朝阁副阁主  "; break;
        case 8: str+="梦回唐朝阁门主    "; break;
        case 7: str+="梦回唐朝阁阁主护卫  "; break;
        case 6: str+="梦回唐朝阁资深长老  "; break;
        case 5: str+=info["info1"]+"分堂主    "; break;
        case 4: str+=info["info1"]+"副堂主    "; break;
        case 3: str+="梦回唐朝阁护法      "; break;
        case 2: str+="梦回唐朝阁宫女      "; break;
...
*/
                 ...
                 else
                {
                 ...
                 mark->move(ob);
                 ob->add("channels", ({"datang"}));
                 ob->add("club", "梦回唐朝阁");
                 ob->add("title", HIY"梦回唐朝阁" NOR);
                 ob->save();
                }
        return 1;
}
...
int do_givepower(string arg)
{
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        ...
                        if(level > 9 || level < 1 )
                                return notify_fail("目前等级分为 <1:仆人> <2:宫女> <3:护法> <4:副堂主> <5:堂主> <6:长老> <7:阁主护卫> <8:少门主> <9:副阁主> <10:阁主> \n");
                        if( level > ob_info["level"] )
                        {
                                type="提升";
                                res ="并和$n握手勉励一番";
...
int do_help(string arg)
{
 if(arg=="datang")
 {
  write(@HELP
       ...
       │  dismiss <人名>              │取消某人分堂职务  │阁主护卫专用│
       │  accept <人名>               │让某人加入帮会    │副阁主专用  │
       │  kickout <人名>              │将某人逐出帮会    │副阁主以上  │
       │  subleader <人名> to <分堂名>│升级某人为分堂主  │少门主以上  │
       │  retire <人名>               │撤除某人分堂主职位│长老以上    │
       │  givepower <人名> to <等级>  │改变某人的职位    │副阁主以上  │
       │  handover <人名>             │掌门职务交接      │阁主专用    │
       │  listmember                  │帮会名册          │阁主护卫以上│
       └───────────────┴─────────┴──────┘
