...
#include <ansi.h>
#include <weapon.h>
#define CLUB_NAME "碧海阁"
#define CLUB_ID "bihaige"
#define CLUB_CHANNEL "bihaige"
#define CLUB_MARK "bihaige necklace"
inherit SWORD;
...
void create()
{
         set_name(HIM"碧海阁的信物"NOR,({"bihaige necklace","necklace","club_mark","sword","jian"}));
        set_weight(0);
...
一块天青色的用上古玄铁打造的令牌，左刻一青龙，右刻一白虎，
你可以用(help bihaige)来查碧海阁指令。若想了解碧海阁的一切
的历史可以用(about bihaige)来取得进一步的资料。
...
string show_level(object user)
{
 ...
 mapping info;
 string str;
 if(wizardp(user)) return "【荣誉会员】碧海阁细作  ";
 ...
 switch(rank)
 {
        case 10: str+="碧海阁帮主      "; break;
        case 9: str+="碧海阁总护法  "; break;
        case 8: str+="碧海阁右护法    "; break;
        case 7: str+="碧海阁左护法  "; break;
        case 6: str+="碧海阁堂主  "; break;
        case 5: str+=info["info1"]+"总管    "; break;
        case 4: str+=info["info1"]+"剑仙    "; break;
        case 3: str+="碧海阁剑神      "; break;
        case 2: str+="碧海阁剑客      "; break;
        default: 
                if(info["info1"]!="无")
                {
...
*/
                if(sizeof(ob_info))
                {
                        if(me_info["level"] != 5 ) return notify_fail("只有总管能将帮众收入分堂。\n");
                        if(!stringp(sub=me_info["info1"])) return notify_fail("只有总管能将帮众收入分堂。\n");
...
int do_givepower(string arg)
{
        ...
        else
        {
                ...
                if(sizeof(ob_info))
                {
                        ...
                        if(level > 9 || level < 1 )
                                return notify_fail("目前等级分为 <1:刺客> <2:剑客> <3:剑神> <4:剑仙> <5:总管> <6:堂主> <7:左护法> <8:右护法> <9:总护法> <10:帮主> \n");
                        if( level > ob_info["level"] )
                        {
                                type="提升";
                                res ="并和$n握手勉励一番";
...
int do_help(string arg)
{
 if(arg=="bihaige")
 {
  write(@HELP
       ...
       │  members                     │列出线上帮会成员  │刺客        │
       │  leaveclub                   │主动退出帮会      │刺客        │
       │  include <人名>              │将某人收入分堂    │总管专用    │
       │  dismiss <人名>              │取消某人分堂职务  │左护法专用  │
       │  accept <人名>               │让某人加入帮会    │总护法专用  │
       │  kickout <人名>              │将某人逐出帮会    │总护法以上  │
       │  subleader <人名> to <分堂名>│升级某人为分堂主  │右护法以上  │
       │  retire <人名>               │撤除某人分堂主职位│堂主以上    │
       │  givepower <人名> to <等级>  │改变某人的职位    │总护法以上  │
       │  handover <人名>             │掌门职务交接      │帮主        │
       │  listmember                  │帮会名册          │左护法以上  │
       └───────────────┴─────────┴──────┘
         ◎ 使用 accept 指令必须对方先用 joinclub 指令申请才能生效。
         ◎ 碧海阁目前有四个分堂: 
         【寒雨堂】、【烈日堂】、【冷月堂】、【狂风堂】
        
         ◎ 等级分为: <1:刺客> <2:剑客> <3:剑神> <4:剑仙> <5:总管> <6:堂主>
                　　　<7:左护法> <8:右护法> <9:总护法> <10:帮主>。
