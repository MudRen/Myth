...
// 50      res = (int) master() -> valid_read(fname, this_object(), "read_file");
    res = (int) sscanf(fname, "/u/%s", socket_info[ fd ][ USER_NAME ]);
	seteuid( getuid() );
...
int check_valid_write( string fname, int fd )
{
	...
	seteuid( socket_info[ fd ][ USER_NAME ] );
// 99      res = (int) master() -> valid_write(fname, this_object(), "write_file");
    res = (int) sscanf(fname, "/u/%s", socket_info[ fd ][ USER_NAME ]);
	seteuid( getuid() );
...
//362                                    stats[2] ? 'x' : '-', /* 'x' if loaded, else ' ' */
                                  '-', /* for bugs */
