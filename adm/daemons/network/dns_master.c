...
string start_message()
{
        return sprintf( "||MUDNAME:%s||NAME:%s||VERSION:%s||DRIVER:%s||MUDLIB:%s"
                "||HOST:%s||PORT:%d||PORTUDP:%d||TIME:%s||ENCODING:%s||USERS:%d||TCP:%s||UPTIME:%s",
                ...
                sizeof(users()), TCP_SERVICE_LEVEL,CHINESE_D->chinese_time(uptime()));
}
...
void create()
{
        ...
        this_host = ([
                ...
                "USERS"         : sprintf("%d",sizeof(users())),
                "UPTIME"        : CHINESE_D->chinese_time(uptime()),
        ]);
