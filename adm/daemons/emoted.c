...
string query_save_file() { return DATA_DIR + "static/emoted"; }
...
varargs mixed do_emote(object me, string verb, string arg, int channel_emote, string who, string default_color)
{
        ...
        object target, *obs, *wiz, *non_wiz;
        string no_emote, can_emote;
        mapping def;
        mapping rwho = ([ ]);

        if( !environment(me) ) return 0;
        // player emote will show their id ...mudring
        obs = all_inventory(environment(me));
        obs = filter_array(obs , (: userp($1) :));
        wiz = filter_array(obs ,(: wizardp($1) :));
        non_wiz = obs - wiz; 
        ...
        if (channel_emote && channel_emote == 4)
        {
                ...
                {
                        str = replace_string(str, "$N", sprintf("%s(%s@%s)", my_name, capitalize(me->query("id")), 
        ...
        if (objectp(target) && stringp(str = def["target"]))
        {
                ...
                if (! channel_emote) {
                        // wizard can see the id who do emote ...mudring
                        if(wizardp(target) && userp(me)) message("emote", WHT +
                                  "["+capitalize(getuid(me))+"]"+CYN + str + NOR, target);
                        else
        ...
        if (stringp(str = def["others" + msg_postfix]))
        {
                ...
                if (! channel_emote)
                {
                        ...
                        message("emote", CYN + str + NOR, non_wiz, ({me, target}));
                         if (userp(me)) message("emote", WHT + "["+capitalize(getuid(me))+"]"+CYN + str + NOR, wiz, ({target,me}));
                                else message("emote", CYN + str + NOR, wiz, ({target,me}));
                }
                else return str;
        }
