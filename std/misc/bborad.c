...
string short()
{
        ...
        if( unread )
                return sprintf("%s [ "HIR"%d"NOR CYN" 张留言，"HIR"%d"NOR CYN" 张未读"NOR" ]", ::short(), sizeof(notes), unread);
        else;                return sprintf("%s [ "HIR"%d"NOR CYN" 张留言"NOR" ]", ::short(), sizeof(notes));
}
string long()
{
        mapping *notes;
...
int do_post(string arg)
{
    ...
    object me;
        ...
        if(! wizardp(me) && (me->query_skill("literate",1))<10)  return notify_fail("还是先去学点读书写字吧！\n");
