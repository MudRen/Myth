...
#define MYFILE "/obj/zhangmen/shushan.c"
  ...
  string week, month, year, day, time; 
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		message_vision ("$N翻身下拜，连声佩服！\n",me);
                if(random(40)>36) {ob->set_temp("shushantop",1); 
                           message_vision ("$N对$n脑袋点了点。\n$N说道：授你一招秘诀！\n",me,ob); 
                          }
        ...
        if( me->query("family/family_name") == "蜀山剑派")
        {
                if( exp <= 100000 ){
                       ...
                       if(me->query("gender") == "女性"){       
                        me->set("title", "蜀山剑派女剑士");
                       }
                       else me->set("title","蜀山剑派剑士");
                } else if(exp <= 2000000 ){
                       if(me->query("gender") == "男性"){
                           me->set("title", HIY"蜀山剑派玄武剑使"NOR);
                       }
                       else me->set("title",HIR"蜀山剑派朱雀剑使"NOR);
                } else if(exp <= 3000000 ){
                        if(me->query("gender") == "女性"){
                           me->set("title", HIW"蜀山剑派白虎剑神"NOR);
                       }
                       else me->set("title",HIG"蜀山剑派青龙剑神"NOR);
                } else {
                        me->set("title", HIC"蜀山剑派剑圣"NOR);
                }
