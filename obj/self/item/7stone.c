...
void create()
{
	set_name(HIC"七"HIR"色"HIG"神"HIW"石"NOR, ({ "7color stone", "stone" }) );
	set_weight( 10000 );
	if( clonep() )
		set_default_object(__FILE__);
	else {
		set("unit", "块");
		set("long", "这是一块有着七彩颜色的神石，里面含有一些稀有的物质。\n");
		set("value", 100);
...
int do_enchase(string arg)
{
   ...
   if(!in_obj->query("weapon_prop/damage")) {   //防具
	...
	HIB"只见"+ob->name()+HIB"化做一道白光,钻入了"+in_obj->query("name")+HIB"里面,不见了！ \n"NOR);
       in_obj->add("armor_prop/armor",5+random(5));
       in_obj->add("armor_prop/dodge",5+random(5));
	...
	HIB"只见"+ob->name()+HIB"化做一道白光,钻入了"+in_obj->query("name")+HIB"里面,不见了！ \n"NOR);
       ...
       in_obj->add("weapon_prop/damage",2 + random(15)); //增加武器伤害度
