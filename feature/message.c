...
string prompt()
{
    ...
    break;
    case "hp":
    p = HIG + sprintf("%d/%d", query("sen"), query("kee")) + p;
    break;
    case "path":
...
void receive_message(string msgclass, string msg)
{
    ...
    if (msgclass == "telnet")
    {
        if (query("env/big5")) msg = CONVERT_D->b5_gb(msg);
        receive(msg);
        return;
    }
    ...
    if (sscanf(msgclass, "%s:%s", subclass, msgclass) == 2)
    {
        ...
        case "channel":
            ...
            break;
        case "wizard":
            if (msgclass != me->query("id") && msgclass != wizhood(me))
                return;
        case "outdoor":
    ...
    if (subclass == "wizard" && !wiz_level(me))
        return;
    ...
    if (query_temp("block_msg/" + subclass))
        return;

    if (query_temp("big5")) msg = CONVERT_D->gb_b5(msg);
