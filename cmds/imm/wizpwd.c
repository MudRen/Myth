...
int main(object me, string arg)
{
     ...
     if (! SECURITY_D->valid_grant(me, "(immortal)")) return 0;
        ...
        input_to("get_old_pass", 1, ob);
            log_file("static/WIZPWD", sprintf("%s changed wizpwd on %s from %s.\n", 
                  getuid(me), log_time(), query_ip_number(me) ));
        return 1;
}
...
private void get_old_pass(string pass, object ob)
{
        ...
        write("\n");
        old_pass = ob->query("wiz_password");
...
private void confirm_new_pass(string pass, object ob, string new_pass)
{
        ...
        seteuid(getuid());
        if( !ob->set("wiz_password", new_pass) ) {
                write("密码变更失败！\n");
                return;
        }
         
        ob->set("wiz_password",new_pass);
        ob->save();
        write("密码变更成功。\n");
}
