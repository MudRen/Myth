...
if(who->query("HellZhenPass")){
   ...
   telling2(who);
if(who->query("death")){
        write ("上次死亡损失"+COMBAT_D->chinese_daoxing(who->query("death/dx_loss"))+"道行。\n");
        write ("上次死亡损失"+who->query("death/combat_exp_loss")+"武学。\n");
        write ("上次死亡损失"+who->query("death/pot_loss")+"潜能。\n");
...
if(who->query("child")){
  ...
  write ("────────────────────────────────────\n");
   write ("一共完成了"+who->query("dajiunpc")+"次保护任务。\n");
   write ("一共搭救了"+who->query("baohuhuaxian")+"个花仙。\n");
