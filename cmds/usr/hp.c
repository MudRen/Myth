...
int main(object me, string arg)
{
        ...
        printf(NOR CYN"┃"NOR WHT"〖 道德 〗%s%21-d "NOR,HIY,(int)ob->query("morality"));
        printf(NOR WHT"  〖 杀气 〗%s%20-d "NOR CYN"   ┃\n"HIG,HIR,(int)ob->query("bellicosity"));
                if (ob->query("degree"))
                {
        printf(NOR CYN"┃"NOR WHT"〖 官拜 〗%s%32-s " NOR "     "WHT"〖 官职 〗%s%5-d "NOR"           "CYN"       ┃ "NOR"\n" NOR,       HIW,ob->query("degree"),HIW,(int)ob->query("office_number"));
                    }
                else
                {
        printf(NOR CYN"┃"NOR WHT"〖 官拜 〗%s%18-s " NOR "     "WHT"〖 官职 〗%s%5-d "NOR"           "CYN"       ┃ "NOR"\n" NOR,       HIW,"未入仕途",HIW,(int)ob->query("office_number"));
                }
        printf(NOR CYN"┃"NOR WHT"〖 等级 〗%s%46-d          " NOR CYN"  ┃\n"NOR,   HIY,my["level"]);
