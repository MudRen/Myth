...
int main(object me, string arg)
{
        ...
        if (! arg)
                return notify_fail("组盟前先想好一个名字吧！\n");
        ...
        if (pure_name!=me->query_temp("teamname"))
               return notify_fail("请你向巫师申请名字,巫师指令set_temp(teamname)。\n");
        ...
        foreach (tob in t)
        {
                ...
                if (! userp(tob))
                        return notify_fail("你想组盟，只怕" + tob->name(1) + "不答应。\n");
...
int do_right(object me, object ob)
{
        ...
        if (! arrayp(t))
                return notify_fail("人家现在已经不打算组盟了。\n");

        if (member_array(me, t) == -1)
                return notify_fail("你现在已经不在人家的组盟考虑范围之内了。\n");
        ...
        if (sizeof(t) == sizeof(r) && ! sizeof(t - r) && ! sizeof(r - t))
        {
                ...
                {
                case 0:
                        msg = "听说" + ob->name(1) + "组建了" +
                              party_name + "，纵横江湖。";
                        break;
                case 1:
                        msg = "据闻" + ob->name(1) + "近日聚众，" +
                              "组成了" + party_name + "，震动江湖。";
                        break;
                default:
                        msg = "传说" + ob->name(1) + "结合人众，" +
                              "组盟成帮，共创" + party_name + "。";
                        break;
                }
