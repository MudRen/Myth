...
void order_list (int count, object *list, object me)
{
  ...
  reset_eval_cost();
  for (i = 0; i < count; i++)
      table[i] = (int)list[i]->query("potential");
  i = count - 2;
...
int main(object me, string arg)
{
  ...
  if (!arg || sscanf(arg,"%d",exp) != 1)
    return notify_fail("指令格式：potsearch <potential>\n");
...
{
write(@HELP
指令格式： potsearch <potential>

查找所有潜能大于你输入的数字的所有玩家（包括在线和不在线）！
HELP
  );
    return 1;
}
