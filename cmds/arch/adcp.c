// cp.c
// created by Doing
// added log and monitored by mudring...
...
int main(object me, string arg)
{
        ...
        {
                if (me && ! SECURITY_D->valid_grant(me, "(arch)"))
                {
        ...
        if (! copy_dir)
        {
                ...
                MONITOR_D->report_adm_object_msg(me,
                  HIR " Cp " + src + " to " + dst + "..." NOR);
                        write("Ok.\n");
                log_file("cmds/edit/"+geteuid(me), 
                sprintf("Cp %s to %s at %s from %s\n",
...
int copy_dir(string src, string dst, int dir_existed)
{
        ...
        if (! SECURITY_D->valid_grant(previous_object(), "(arch)"))
                return 0;
